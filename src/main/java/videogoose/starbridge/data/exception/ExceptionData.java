package videogoose.starbridge.data.exception;
...
public class ExceptionData {
	...
	public ExceptionData(String exceptionClassName, String[] stacktrace) {
		...
		for(ExceptionData exception : exceptions) {
			...
			float similarity = (float) similarLines / exception.getStacktrace().length;
			if(similarity >= 0.9f) { //This is a known exception
				name = exception.name;
				description = exception.description;
				modName = exception.modName;
				modVersion = exception.modVersion;
				this.exceptionClassName = exceptionClassName;
				this.stacktrace = stacktrace;
				//This exception has occurred before, increase the severity
				severity = exception.severity + 1;
				...
				name = "New Exception in " + modName + "v" + modVersion;
				description = "This is a new exception that has not been added to the exception database yet. Please report this exception to the mod author.";
				PersistentObjectUtil.addObject(StarBridge.getInstance().getSkeleton(), this);
			}
		}
